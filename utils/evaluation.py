...
def evaluate_model(loader_dict, model, device, model_name) -> pd.DataFrame:
    ...
    return pd.DataFrame(
        {"y_true": y_true, "y_pred": y_pred, "split": split_arr, "model": model_name}
    )


def regression_metrics(
    y_true: np.ndarray, y_pred: np.ndarray, num_nodes: int = 6
) -> dict:
    """
...
def visualize_compliance_matrices_dual(
    model_attn: torch.nn.Module,
    model_gcn: torch.nn.Module,
    dataset: list,
    device,
    num_examples=3,
    fmt=".2f",
    ...
    for i, (t, a, g) in enumerate(zip(true_mats, attn_mats, gcn_mats)):
        ...
        ax_t.set_title(f"True #{i+1}", fontsize=9)
        ax_t.set_xticks([])
        ax_t.set_yticks([])
        im_a = sns.heatmap(
            a,
        ...
        ax_a.set_title("Attention", fontsize=8)
        ax_a.set_xticks([])
        ax_a.set_yticks([])
        im_g = sns.heatmap(
            g,
        ...
        ax_g.set_title("GCN", fontsize=8)
        ax_g.set_xticks([])
        ax_g.set_yticks([])
        if im_for_cbar is None:
            im_for_cbar = im_t.collections[0]
    ...
    fig.suptitle(
        "Compliance Matrices: True vs Attention vs GCN", fontsize=12, weight="bold"
    )
    plt.show()
